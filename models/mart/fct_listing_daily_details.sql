...
with
    calendar as (select * from {{ ref("int_calendar_enriched") }}),

    dim as (select * from {{ ref("dim_listings") }}),

    amenities as (select * from {{ ref("int_amenities_history") }}),

    joined as (
        select
            {{ dbt_utils.generate_surrogate_key(["c.listing_id", "c.calendar_date"]) }}
            as fact_id,
            c.calendar_date,
            c.listing_id,
        ...
        from calendar c
        left join dim d on c.listing_id = d.listing_id
        left join
            amenities a
